...
u_db_restore_last() {
  ...
  u_db_restore "$(u_fs_get_most_recent $ASC_DB_DUMPS_BASE_PATH/local/$p_db_id)" "$p_db_id" "$2"
}
  ...
  if [[ -n "$p_option" ]]; then
    case "$p_option" in
      ...
      initial)
        ...
        # u_fs_file_list "$ASC_DB_DUMPS_BASE_PATH/local/$p_db_id" 'initial.*'
        u_fs_file_list "$ASC_DB_DUMPS_BASE_PATH" 'initial.*'
        for initial_dump_match in $file_list; do
          # dump_to_return="$ASC_DB_DUMPS_BASE_PATH/local/$p_db_id/$initial_dump_match"
          dump_to_return="$ASC_DB_DUMPS_BASE_PATH/$initial_dump_match"
        done
        ;;
    esac
  else
    dump_to_return="$(u_fs_get_most_recent $ASC_DB_DUMPS_BASE_PATH/local/$p_db_id)"
  fi
