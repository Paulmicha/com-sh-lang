...
global DRUSH_DEFAULT_URI "[default]='$INSTANCE_DOMAIN' [help]='Some commands may need to output absolute URLs, like drush uli, and sometimes the domain cannot be generated properly without passing it as the --uri argument. So this var contains the default uri to use. For multi-site Drupal setups, see the extension : drupalwt'"
