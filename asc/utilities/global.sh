...
u_global_read_key() {
  ...
  local prefix_delimiter="$(u_asc_common_val globals-key-prefix)"
  local tmp_space_placeholder="$(u_asc_common_val globals-tmp-space-placeholder)"
  ...
  # @see u_global_read_key()
    ...
    for sub_key in $sub_keys; do
      if [[ -n "${GLOBALS[$p_var|$sub_key]}" ]]; then
        ...
        sub_val="$(u_str_replace ' ' "$(u_asc_common_val globals-tmp-space-placeholder)" "$sub_val")"
