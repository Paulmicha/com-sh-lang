  ...
  for stack_extension in "${STACK_SERVICES[@]}"; do
    ...
    if [[ -n "${sp_arr[1]}" ]]; then
      for sp_type in $sp_types; do
        u_global_get_includes_lookup_version "asc/$sp_type/extensions/${sp_arr[0]}" "${sp_arr[1]}" true
      done
    else
      for sp_type in $sp_types; do
        u_array_add_once "asc/$sp_type/extensions/${stack_extension}/vars.sh" GLOBALS_INCLUDES_PATHS
        u_autoload_add_lookup_level "asc/$sp_type/extensions/${stack_extension}/" 'vars.sh' "$PROVISION_USING" GLOBALS_INCLUDES_PATHS
      done
    fi
  done
