...
The very first step required to use ASC is writing env settings (**stack init**) :
...
## Start with common ASC variables (target : host-level deps)

Loaded from `asc/env/dist/env.vars.sh`, contains the fundamental value `$PROJECT_STACK` used to derive the rest of conditional settings. Examples :
