...
The variables written and loaded using the scripts in this directory are used in all ASC tasks. Some are common to any type of stack, while others are specific to certain variants.
...
- add the common, generic `asc/env/dist/env.vars.sh` model
...
Every time `asc/stack/init.sh` is called, current instance's env file is (over)written. The fundamental value `$PROJECT_STACK` is used to derive the rest of conditional settings and custom overrides/complements lookup paths.
...
Given the following value for the '-s' or '--stack' arg: "drupal-7", and for the '-p' or '--provision' arg: "scripts", this script will attempt to copy the contents of all the following files (if they exist), append it to the file storing settings of the current local instance, `asc/env/current/.app.vars.sh` :

- `asc/env/dist/drupal/app.vars.sh.dist`
- `asc/env/dist/drupal/scripts.provision.vars.sh.dist`
- `asc/env/dist/drupal/7/app.vars.sh.dist`
- `asc/env/dist/drupal/7/scripts.provision.vars.sh.dist`
