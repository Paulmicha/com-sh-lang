...
global APP_GIT_ORIGIN "[help]='Optional (leave blank to skip). Ex: git@my-git-origin.org:my-git-account/asc.git. Allows projects to have their own separate repo. If set, then by default \"instance init\" will clone that repo (once) and a default selection of Git hooks will be overwritten to trigger ASC hooks.'"
