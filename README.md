...
ASC core provides the followig globals by default (see `asc/env/global.vars.sh`, and `asc/utilities/global.sh` for details about the `global()` function), which show the syntax to provide default values and optional help text that will be displayed when user input is prompted during *instance init* when the `-y` flag is not set (otherwise it won't prompt for anything and just use the default value) :
